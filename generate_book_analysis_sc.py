...
DATABASE_NAME = 'ai_book_analysis_sc.db'
...
def insert_introduction(conn, book, section, content, update=False):
    """
...
def fetch_tc_introductions():
    db = os.path.join(os.getcwd(), "ai_book_analysis_tc.db")
    with apsw.Connection(db) as connn:
        cursor = connn.cursor()
        cursor.execute("SELECT * FROM Introduction ORDER BY Book, Section")
...
if __name__ == '__main__':
    ...
    if db_connection:
        for b, c, content in fetch_tc_introductions():
            print("Working on verse:", b, c)
            content_sc = convert_traditional_chinese(content, print_on_terminal=False)
            insert_introduction(db_connection, b, c, content_sc, update=False)
